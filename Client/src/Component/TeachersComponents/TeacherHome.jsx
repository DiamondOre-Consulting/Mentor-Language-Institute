...
const TeacherHome = ({ teacherData }) => {
  ...
  // useEffect to fetch courses only once
  useEffect(() => {
            ...
            <div className="grid grid-cols--1 md:grid-cols-3 gap-4 ">
              {loading ? (
                ...
                classesData.map((course) => (
                  <div
                    className=" border rounded-md border-0 shadow-xl hover:shadow-none cursor-pointer"
                    key={course._id}
                  >
                    <div className="px-2 py-3 col-span-1 bg-orange-500 rounded-md">
                      ...
                      <Tooltip content="Click To Edit Hours">
                        <Button>
                          <span
                            className="text-sm text-gray-50 -ml-5 -mt-3"
                            onClick={() => handleveiw(course._id)}
                          >
                            Total hours{" "}
                            <span className="bg-gray-50 text-bold text-black px-1 rounded-full">
                              {course.totalHours}
                            </span>
                          </span>
                        </Button>
      ...
      {showPopup && (
        <div className="fixed inset-0 flex items-center justify-center">
          ...
          <section className="absolute bg-black top-10 right-10 rounded-lg shadow-xl w-64 p-2 text-center">
            <h2 className="text-md font-semibold opacity-100 text-gray-100">
      ...
      {showPopupCourses && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <section className="rounded-lg shadow-xl bg-white w-4/5 sm:w-3/5 lg:w-1/4 relative">
            <svg
              className="h-5 w-5 bg-red-600 cursor-pointer p-1 text-2xl rounded-full text-gray-50 absolute top-0 right-0 m-2"
              ...
              <div className="flex  items-center justify-between mt-2">
                <div>
                  <Link
                    ...
                    className="p-2 bg-orange-500 rounded-md text-gray-100 cursor-pointer"
                  >
                    My Students
                  </Link>
                </div>
      ...
      {showScheduleClass && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <section className="rounded-lg shadow-xl bg-white w-4/5 sm:w-3/5 lg:w-1/3">
            <svg
              class="h-5 w-5 bg-red-600 cursor-pointer p-1 rounded-full text-gray-100 float-right mr-1 mt-1 "
              onClick={handleCloseCourses}
              width="24"
              height="24"
              viewBox="0 0 24 24"
            ...
            <div className="p-6 text-left">
              <p className="font-bold text-xl md:text-3xl text-black">
                {oneClassDetails.classTitle}
              </p>
              <h2 className="text-normal font-bold text-teal-green-900 mb-1">
                {" "}
                Select Date
              </h2>
              <input
                ...
                value={date ? date.split("-").reverse().join("-") : ""} // Bind the value to the date state variable
      ...
      {showUpdateHoursPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <section className="rounded-lg shadow-xl bg-white w-4/5 sm:w-3/5 lg:w-1/4 relative">
            <svg
              className="h-5 w-5 bg-red-600 cursor-pointer p-1 text-2xl rounded-full text-gray-50 absolute top-0 right-0 m-2"
              onClick={() => setShowUpdateHoursPopup(false)}
              width="24"
              height="24"
              viewBox="0 0 24 24"
              stroke-width="2"
